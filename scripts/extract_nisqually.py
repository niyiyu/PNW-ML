...
window_length = 240                                                      # window length in seconds
nbucket = 1

def update_data(data, streamdata, ibucket):
    if streamdata.shape[0] < 3:
        for _ in range(3 - len(streamdata)):
            streamdata = np.concatenate((np.zeros([1, 15001]), streamdata)) # XXZ
    streamdata = np.expand_dims(streamdata, axis = 0)                       # ENZ
...
event = obspy.read_events(quakeml_path + '2001/uw10530748.xml')
...
for pick in event[0].picks:
    ...
    onset = pick.onset
    if trace_channel[:2] in ['EH', "BH", "HH", 'EN']: # limit the instrument type
        station_latitude_deg = "TBA"
        station_longitude_deg = "TBA"
        station_elevation_m = "TBA"
        phase = pick.phase_hint
        trace_start_time = source_origin_time - 60
            ...
            if CODE in meta['CODE'].unique():
                ...
                if len(stream) > 0 and len(stream.get_gaps()) == 0:
                    ...
                    if stream_data.shape[1] == window_length * trace_sampling_rate_hz + 1:
                        ...
                        break
                    trace_name = 'bucket%d$%d,:3,:30001' % (ibucket, len(data[ibucket]) - 1)
...
# select = (pd.notnull(meta['trace_S_arrivzal_sample'])) & (pd.notnull(meta['trace_P_arrival_sample']))
meta_PS = meta
# meta_PS = meta_PS.drop(columns = "CODE")
meta_PS = meta_PS.drop(columns = "trace_S_polarity") # dont rely on S polarity
meta_PS.to_csv("../product/nisqually_metadata.csv", sep = ',', index=False)

f = h5py.File("../product/nisqually_waveforms.hdf5", mode = "w")
