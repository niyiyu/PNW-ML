...
for quake_year in np.arange(2002, 2022):
    if quake_year % size == rank:
            ...
            if (ievent % 10 == 1) or (ievent % 10 == 2):
                ...
                for pick in event[0].picks:
                    ...
                    if trace_channel[:2] in ['BH', "HH", "EH"]: # limit the instrument type
                        ...
                        trace_start_time = source_origin_time - 300
                        ...
                        try:
                            ...
                            if CODE in meta['CODE'].unique():
                                ...
                                if len(stream) > 0 and len(stream.get_gaps()) == 0:
                                    ...
                                    meta.iloc[idx, meta.columns.get_loc("select")] = "yes"
                            else:
                                meta = meta.append({
        ...
        select = (pd.notnull(meta['select']))
        meta_selected = meta[select]
        meta_selected = meta_selected.drop(columns = "CODE")

        if len(meta_selected) >0:
            meta_selected.to_csv("../data/mpiextract2/proc%s_metadata.csv" % str(quake_year), sep = ',', index=False)
            
            f = h5py.File("../data/mpiextract2/proc%s_waveforms.hdf5" % str(quake_year), mode = "w")
